...
if ($scfsub eq 1){
	for (my $state=0; $state<=$numfiles-1; $state++){
		...
		chdir("STATE_$state");
		for (my $i=$xstart; $i<=$xend+$xinc; $i+=$xinc){
			for (my $j=$ystart; $j<=$yend+$yinc; $j+=$yinc){
	        		my $gjf = "$molecule"."_sol"."$state"."_x_".sprintf("%.5f",$i)."_y_".sprintf("%.5f",$j);
...
if ($proj_state ne -1){
	...
	for (my $i=$xstart; $i<=$xend+$xinc; $i+=$xinc){
		for (my $j=$ystart; $j<=$yend+$yinc; $j+=$yinc){
	        	my $gjf = "$molecule"."_PROJ_x_".sprintf("%.5f",$i)."_y_".sprintf("%.5f",$j);
	        	open  (GJF,">$gjf.input") or die "Could not create $gjf.\n";
			print GJF "$curdir/STATE_$proj_state/$molecule"."_sol"."$proj_state"."_x_".sprintf("%.5f",$i)."_y_".sprintf("%.5f",$j).".mat\n";
	        	close GJF;
...
if ($berry_state ne -1){
	...
	for (my $i=$xstart; $i<=$xend+$xinc; $i+=$xinc){
		for (my $j=$ystart; $j<=$yend+$yinc; $j+=$yinc){
	        	my $gjf = "$molecule"."_NOCI_x_".sprintf("%.5f",$i)."_y_".sprintf("%.5f",$j);
	        	open  (GJF,">$gjf.input") or die "Could not create $gjf.\n";
			print GJF "$numfiles\n";
			for (my $state=0; $state<=$numfiles-1; $state++){
				print GJF "$curdir/STATE_$state/$molecule"."_sol"."$state"."_x_".sprintf("%.5f",$i)."_y_".sprintf("%.5f",$j).".mat\n";
			}
	        	close GJF;
			open (OUT,">$gjf.out") or die "Could not open $gjf.out.\n";
